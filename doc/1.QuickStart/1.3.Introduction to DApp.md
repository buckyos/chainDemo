...
* `run.js`: 执行在`src/coin/chain.json`里配置的chainSDK命令行的封装脚本。
    * 通过`node run.js -chain ${chainName} -action ${actionName}`来自执行。
    * 例如，上一节我们执行的`node run.js -chian coin -action create`等。
...
4. `chian.json`里配置了运行BlockChain各个节点的命令行和对应的参数。
...
我们知道创建一个Block，会获得一定的coin奖励，创建创世块也不例外。genesis.json里的`coinbase`指定了创造创世块时，给谁发奖励。
...
* `ErrorCode`：全局错误码定义
* `BigNumber`：数值运算
* `ValueViewContext`：合约状态(`storage`)的只读上下文，通过该接口只允许调用合约的只读接口。参考[ref_context/ValueViewContext](../4.References/ref_contxt.md)。
* `ValueTransactionContext`：合约状态(`storage`)的可读可写上下文，通过该接口允许读写合约的相关状态。参考[ref_context/ValueTransactionContext](../4.References/ref_contxt.md)。
...
#### 测试一下
在chainDemo根目录下，我们执行下命令测试下coin这个DApp的测试用例：
```
node run.js -chain coin -action test
```
...
```TypeScript
function runCmd(cmd: string) {
    let chain = runEnv; // eval里可以通过闭包调用chian的接口。
    try {
        eval(cmd);
    } catch (e) {
        console.error(e.message);
...
可以看到`program`指定要执行的程序，`chain_host`是ChainSDK内置的命令，所有参数的信息都可以在参考手册里找到，主要用来：
* 创建创世块
* 启动挖矿节点
* 启动同步节点
