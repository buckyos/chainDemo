...
我们知道创建一个Block，会获得一定的coin奖励，创建创世块也不例外。genesis.json里的`coinbase`指定了创造创世块时，给谁发奖励。更多`genesis.json`的配置，参考[tool_host](../4.References/tool_host)里的`genesis config字段`一节。
